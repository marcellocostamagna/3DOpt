...
The folder `Explicit_Hs_analysis` contains scripts for analyzing the impact of implicit vs. explicit hydrogens in SMILES on 3D structure generation.
It provides single-run and multi-run workflows comparing canonical CSD SMILES (implicit H) with explicit-H SMILES (from RDKit) as inputs to CCDC, RDKit, and Open Babel.